...
if st.button('Obtener Predicción'):
    ...
    # URL de la API de predicción (cambia esta URL si la API no está en
    # esa url)
    api_url = os.environ.get('API_URL', 'http://localhost:5000/predict')

    try:
        # Enviar la petición a la API
        response = requests.post(api_url, data=json.dumps(payload), headers={'Content-Type': 'application/json'})7
